...
type Params = { type: string; pageNumber: string };

export const generateMetadata = async ({ params }: { params: Promise<Params> }): Promise<Metadata> => {
  const { type: rawType, pageNumber: rawPage } = await params;
  ...
  const title = getPostTypePageTitle(defaultLocale, type, pageNumber);
  const description = getPostTypePageDescription(defaultLocale, type, posts.length, pageNumber);
  ...
  const availableLocales = locales.filter((locale) => {
    const { totalPages } = paginatePostsByType(locale, type, 1);
...
export default async function TypeCollectionPageNumber({ params }: { params: Promise<Params> }) {
  ...
  const description = getPostTypePageDescription(defaultLocale, type, allPosts.length, pageNumber);
    ...
    <BlogLayoutView locale={defaultLocale}>
      <section className="flex flex-col gap-4 rounded-[2rem] border border-[#eadfcd]/70 bg-white/70 p-5 text-sm text-[#4b4139] shadow-[0_24px_70px_-40px_rgba(40,30,20,0.4)] sm:p-8">
        ...
        <h1 className="text-[clamp(1.8rem,4.5vw,2.6rem)] font-semibold leading-tight text-[#2f2b26]">
          {getPostTypePageTitle(defaultLocale, type, pageNumber)}
        </h1>
        <p>{description}</p>
      </section>
